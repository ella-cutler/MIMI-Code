...
void run_demo_seq_A_only (void);
void lamp_test (void);
void run_launch_seq (void);
void test (void);
volatile unsigned char status_N = 0;
volatile unsigned char lamp_test_N = 0;
volatile unsigned char skip = 0;
 int bell_vol = 32768;	//default
int buzzer_vol = 32768;		//default
int pas_vol = 32768;		//default
int vol_increment = 2048;	//default
volatile unsigned int short key_9_down = 0;
void initialize (void);
void All_OFF (void);
void All_ON (void);
void run_seq (void);		// only good for test
volatile unsigned char random_N (void);volatile unsigned char puMSILP;
void bell_on (void);
void buzzer_on (void);
void pas_on (void);
void PASwarble (void);
void training_audio (void);void LLC_audio(void);
...
home ();
	  ...
	  break;
	    ...
	    {			// if 9 key is held for 5 seconds turn off power
	      char buf[16];
...
initialize (void)  { printf ("initializer\r\n");
  setPadDrive (1, 0x05000000);	// bit 4, slew control on 0, slew off 1
  lamp_test_N = 0;
   char buf[31];
  sprintf (buf, "amixer set PCM -- -bell_volume");
...
annunciator (unsigned char WHICH_ONE, unsigned short int WhatsON)
{
  printf ("annunciator\r\r\n");
  if (WHICH_ONE == A_Left)
    {
      A_Left_copy = WhatsON;
    }				// keep a copy of what is in the shift registers
  if (WHICH_ONE == A_Right)
    {
      A_Right_copy = WhatsON;
    }
  if (WHICH_ONE == B_Left)
    {
      B_Left_copy = WhatsON;
    }
  if (WHICH_ONE == B_Right)
    {
      B_Right_copy = WhatsON;
    }
  if (WHICH_ONE == C_Left)
    {
      C_Left_copy = WhatsON;
    }
  if (WHICH_ONE == C_Right)
    {
      C_Right_copy = WhatsON;
    }
  if (WHICH_ONE == D_Left)
    {
      D_Left_copy = WhatsON;
    }
  if (WHICH_ONE == D_Right)
    {
      D_Right_copy = WhatsON;
    }
  if (WHICH_ONE == E_Left)
    {
      E_Left_copy = WhatsON;
    }
  if (WHICH_ONE == E_Right)
    {
      E_Right_copy = WhatsON;
    }
...
unsigned char Mux2_value = (WHICH_ONE & (0b0100)) >> 2;
  ...
   printf ("which one Annunciator   %d%d%d%d\r\n", Mux3_value, Mux2_value,
	     Mux1_value, Mux0_value);
     unsigned char Array[] = { MSB_WhatsON, LSB_WhatsON };
  wiringPiSPIDataRW (0, Array, 2);
    delayMicroseconds (1);	// make sure bus has settled0
...
bell_on (void)
{
  ...
     system ("aplay  /home/pi/RUN/sounds2/bell_2s_rev3.wav");
...
buzzer_on (void)
{
  ...
   system ("aplay  /home/pi/RUN/sounds2/buzzer_2s_rev3.wav");
...
pas_on (void)
{
  ...
   system ("aplay /home/pi/RUN/sounds2/pas_5s_rev3.wav");
...
PASwarble (void)
{
    ...
    // system("aplay /home/pi/RUN/sounds/pas_3s.wav") ;  
    return;
...
training_audio (void)
{
  printf ("playing traning_audio \r\n");
  char buf[24];
     
    // aplay uses 0 to 65536
    sprintf (buf, "amixer set Master %d", pas_vol);
  system (buf);
   system ("aplay /home/pi/RUN/sounds2/training_audio_rev3.wav");
  
    //system("aplay /home/pi/RUN/sounds2/LCC-audio-03-D11-faded.wav");    
    return;
}

voidLLC_audio (void){  printf ("playing LLC_audio \r\n");  char buf[24];         // aplay uses 0 to 65536    sprintf (buf, "amixer set Master %d", pas_vol);  system (buf);   //system ("aplay /home/pi/RUN/sounds2/training_audio_rev3.wav");      system("aplay /home/pi/RUN/sounds2/LCC-audio-03-D11-faded.wav");        return;}  void
home (void)
{
...
security (unsigned char which_MISILP)
{
  ...
  annunciator (which_MISILP,
		STRATEGIC_ALERT | OUTER_SECURITY | INNER_SECURITY);
  buzzer_on ();
   return;
 }

  void
fault (unsigned char which_MISILP)
{
...
not_authenticated (unsigned char which_MISILP)
{
...
anti_jam_mode (unsigned char which_MISILP)
{
    ...
    printf ("anti jam mode\r\n");
		...
		OUTER_SECURITY | NOT_AUTHENTICATED | ANTI_JAM_MODE);
  buzzer_on ();
   return;
 }

 void
missile_away (unsigned char which_MISILP)
{
    ...
    annunciator (which_MISILP, ENABLED | STRATEGIC_ALERT);
		...
		INNER_SECURITY | LAUNCH_IN_PROCESS | ENABLED |
		STRATEGIC_ALERT);
  buzzer_on ();
  delay (((random () % delay_delta) + delay_min));
  annunciator (which_MISILP,
		OUTER_SECURITY | INNER_SECURITY | LAUNCH_IN_PROCESS | ENABLED
		| STRATEGIC_ALERT);
  buzzer_on ();
  delay (((random () % delay_delta) + delay_min));
  annunciator (which_MISILP,
		MISSILE_AWAY | OUTER_SECURITY | INNER_SECURITY |
		LAUNCH_IN_PROCESS | ENABLED | STRATEGIC_ALERT);
  delay (((random () % delay_delta) + delay_min));	// double delay 
  delay (((random () % delay_delta) + delay_min));
...
run_demo_seq (void)
{
  ...
  if (status_N == 0)
    {
      ...
       //pas_on ();status_N++;  // add one now the pas  is off
    }
    ...
    if (status_N == 3)
    {
		    ...
		    LAUNCH_IN_PROCESS | LAUNCH_COMMAND | ENABLED |
		    STRATEGIC_ALERT);
    ...
    if (status_N == 4)
    {
		    ...
		    INNER_SECURITY | LAUNCH_IN_PROCESS | LAUNCH_COMMAND |
		    ENABLED | STRATEGIC_ALERT);
    ...
    if (status_N == 5)
    {
		    ...
		    OUTER_SECURITY | INNER_SECURITY | LAUNCH_IN_PROCESS |
		    ENABLED | STRATEGIC_ALERT);
...
lamp_test (void)
{
	...
	{
	  annunciator (lamp_test_N - 1, STRATEGIC_ALERT);
	}			//leave STRATEGIC ALERT LAMP on
