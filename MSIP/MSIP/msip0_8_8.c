 ...
 * 
 * need a file in the USB Flas drive  /media/pi/USB_MEM/vol.bin
 * the volume setting is saved here.  Did not use sd card in case of corruption
 * 
 * 
 * 
 * 
...
unsigned int short vol = 32768;	//vol is the volume setting
...
initialize (void)  { printf ("initializer\r\n");
  ...
  fp = fopen ("/media/pi/USB_MEM/vol.bin", "ab+");
  fread (&vol, 2, 1, fp);
  printf ("VOL= %d \r\n", vol);
   fclose (fp);
   char buf[32];
  sprintf (buf, "amixer set Master %d", vol);
  system (buf);		// set default volume here
...
All_OFF (void)
{
    ...
    {
      unsigned char Mux3_value = (count & 0 b1000) >> 3;	//step thru all 10 Mux values and write 0s to bits
      unsigned char Mux2_value = (count & 0 b0100) >> 2;
      unsigned char Mux1_value = (count & 0 b0010) >> 1;
      unsigned char Mux0_value = (count & 0 b0001);
...
annunciator (unsigned char WHICH_ONE, unsigned short int WhatsON)
{
  ...
     unsigned char Mux3_value = (WHICH_ONE & 0 b1000) >> 3;	// parse WHICH_ONE into bits
  unsigned char Mux2_value = (WHICH_ONE & 0 b0100) >> 2;
  unsigned char Mux1_value = (WHICH_ONE & 0 b0010) >> 1;
  unsigned char Mux0_value = (WHICH_ONE & 0 b0001);
...
run_demo_seq (void)
{
    ...
    if (status_N == 3)
    {
      annunciator (puMSILP, LAUNCH_IN_PROCESS | ENABLED | STRATEGIC_ALERT);
...
volume_control_down (void)
{
  ...
    fp = fopen ("/media/pi/USB_MEM/vol.bin", "wb");
