...
void audio_chip_setup(void){
	...
	//read_data = wiringPiI2cRead8(fd,reg number);
	wiringPiI2CWrite8(fd, 0x36, 0x41);  //turn on limiter and default i2s sck pol
        wiringPiI2CWrite8(fd, 0x08, 0x35);  //auto processor on right justified 20 bits
	wiringPiI2CWrite8(fd, 0x03, 0x1D);	//power mode profile 3
	wiringPiI2CWrite8(fd, 0x3A, 0x40);	//set volume to -34 dB
