...
/*
	...
	wiringPiI2CWriteReg8(fd, 0x40, 0X20);	//set volume to 
...
void volume_control_down(void){
	...
	volume = wiringPiI2CReadReg8(fd,0x40);
	wiringPiI2CWriteReg8(fd, ++volume, 0x40);  //decrease volume 1 dB
...
void audio ( void *play_this_one){
    ...
    if(*value ==BELL){
    system("mpg123  /home/pi/MSIP/bell.mp3") ;  
   printf(" playing bell\r\n");
 }
 
   if(*value ==BUZZER){
 system("mpg123  /home/pi/MSIP/buzzer.mp3") ;
  ...
  system("mpg123  /home/pi/MSIP/pas.mp3") ;
   printf(" playing pas\r\n");
 }
 
 if(*value ==TRACK ){
 system("mpg123  /home/pi/MSIP/track.mp3") ;
